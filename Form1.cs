...
namespace FileBatchPrinterGUI
{
        ...
        public static List<Dictionary<string, object>> ParseArray(string json)
        {
            ...
            for (int i = 0; i < json.Length; i++)
            {
                ...
                if (depth == 0 && json[i] == '}')
                {
                    ...
                    result.Add(ParseObject(objJson));
                    start = i + 1;
        ...
        private static Dictionary<string, object> ParseObject(string json)
        {
            ...
            while (pos < json.Length)
            {
                ...
                else if (json[pos] == '{')
                {
                    ...
                    value = ParseObject(json.Substring(valueStart, pos - valueStart));
                }
                else if (json[pos] == 't' && json.Substring(pos, 4) == "true")
                {
                    value = true;
                    pos += 4;
                }
                else if (json[pos] == 'f' && json.Substring(pos, 5) == "false")
                {
                    value = false;
                    pos += 5;
                }
                else if (json[pos] == 'n' && json.Substring(pos, 4) == "null")
                {
                    value = null;
                    pos += 4;
                }
            ...
            return await Task.Run(() =>
            {
                try
                {
                    ...
                    using (var client = new WebClient())
                    {
                        ...
                        // 使用自定义 JSON 解析器
                        var devices = SimpleJsonParser.ParseArray(json);
                        ...
                        string deviceCode = device.ContainsKey("device_code") ? device["device_code"]?.ToString() : "";
                        string deviceName = device.ContainsKey("device_name") ? device["device_name"]?.ToString() : "";
                        string expireDateStr = device.ContainsKey("expire_date") ? device["expire_date"]?.ToString() : "";
                        bool isActive = device.ContainsKey("is_active") ? Convert.ToBoolean(device["is_active"]) : true;
                ...
                catch (WebException ex)
                {
                    ...
                    return new LicenseCheckResult { Success = false, Code = "NETWORK_ERROR", Error = $"网络连接失败: {errorMsg}" };
                }
                catch (Exception ex)
                {
    ...
    public class LicenseDialog : Form
    {
        ...
        private void SetupUI()
        {
            ...
            {
                Text = "提示：如果设备未授权，请联系管理员在 Supabase 中添加此设备码。",
